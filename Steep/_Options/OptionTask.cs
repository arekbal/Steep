...
namespace Steep
{
  ...
  {
    internal static readonly Task<Option<T>> NoneTask = Task.FromResult(new Option<T>());
    ...
    public static OptionTask<T> Some(Task<T> val)
    {
      //Task<Option<T>> task;
      //if (val.Is)
      return new OptionTask<T> { _task = val.ContinueWith(task => Option.Some(task.Result)) }; // TODO: get rid of this extra internal conversion
    }
    ...
    public static OptionTask<T> Some(ValueTask<T> val)
    {
      ...
      if(val.IsCompletedSuccessfully)
        return new OptionTask<T> { _val = Option.Some(val.Result) }; // TODO: get rid of this extra internal conversion

      return new OptionTask<T> { _task = val.AsTask().ContinueWith(task => Option.Some(task.Result)) }; // TODO: get rid of this extra internal conversion
    }
  }
}
